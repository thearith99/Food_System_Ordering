  ...
  const createOrder = async (cart, totalAmount) => {
    try {
      const response = await fetch('http://localhost:3000/api/orders', {
        ...
        body: JSON.stringify({ cart, totalAmount }) // Send cart and totalAmount as JSON
      })
      ...
      toast.error('Failed to create order. Please try again later.', toastOptions)
    }
  }
                        ...
                        >
                          <div className='text-cyan-900 cursor-pointer hover:bg-gray-100 text-center border border-cyan-900 rounded-md bg-white h-full flex flex-col justify-center'>
                            <div className='aspect-w-1 aspect-h-1'>
                              <img
                                src={`http://localhost:3000/images/${category.image}.jpg`}
                      ...
                      cart.map((cartProduct, key) => (
                        <tr key={key}>
                          <td className='px-2 py-2'>
                            <img
                              src={`http://localhost:3000/images/${cartProduct.image}.jpg`}
            ...
            <div className='mt-auto'>
              <div className='flex flex-col gap-2'>
                ...
                <div>
                  ...
                  {totalAmount !== 0 ? (
                    <button
                      className='bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-lg py-3 w-full'
                      onClick={() => createOrder(cart, totalAmount)}
                    >
                      Pay Now
                    </button>
                  ) : (
